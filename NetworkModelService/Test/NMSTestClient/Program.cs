			...
			try
			{
				...
				do
				{
                    ...
                    else if(str == "5")
                    {
                        try
                        {
                            List<long> data = tgda.TestGetExtentValuesAllTypes();
                            Console.WriteLine("Number of entities: {0}", data.Count);
                            Console.WriteLine("Entities:");
                            foreach (long id in data)
                            {
                                Console.WriteLine("0x{0:X16}", id);
                            }
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Test failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
